...
export type Status = "草稿" | "待確認" | "生效" | "部分提領" | "已結清" | "已結案" | "已取消" | "已刪除";
export type OutboundStatus = "草稿" | "待覆核" | "已生效" | "已結案" | "已取消" | "已刪除";
...
export function useInventoryStore(){
  ...
  const actions=useMemo(()=>({
    ...
    updateConsignment(id:string,input:Pick<Consignment,"orderId"|"source"|"customerId"|"deadline"|"note"|"lines">,submit:boolean,user:User){setStore(s=>({...s,consignments:s.consignments.map(x=>x.id!==id||x.status!=="草稿"?x:{...x,...input,status:submit?"待確認":"草稿",audits:[...x.audits,{id:makeId("a"),action:"修改單據",actorId:user.id,actorName:user.name,at:new Date().toISOString(),note:submit?"修改後重新送出覆核":"儲存修改內容"}]})}));},
    ...
    setConsignmentStatus(id:string,status:"已取消"|"已結案"|"已刪除",user:User){setStore(s=>({...s,consignments:s.consignments.map(x=>x.id!==id?x:{...x,status,audits:[...x.audits,{id:makeId("a"),action:status,actorId:user.id,actorName:user.name,at:new Date().toISOString(),note:"單據保留，僅變更狀態"}]})}));},
    ...
    updateOutbound(id:string,input:Pick<Outbound,"customerId"|"plannedDate"|"actualDate"|"receiver"|"phone"|"vehicleNo"|"note">&{lines:{productId:string;warehouseId:string;qty:number}[]},submit:boolean,user:User){setStore(s=>{const current=s.outbounds.find(x=>x.id===id);if(!current||current.status!=="草稿")return s;const lines:OutboundLine[]=input.lines.map(l=>({...l,id:makeId("outl"),allocations:fifoAllocations(s,input.customerId,l.productId,l.warehouseId,l.qty)}));return{...s,outbounds:s.outbounds.map(x=>x.id!==id?x:{...x,...input,lines,status:submit?"待覆核":"草稿",audits:[...x.audits,{id:makeId("a"),action:"修改單據",actorId:user.id,actorName:user.name,at:new Date().toISOString(),note:submit?"修改後重新送出覆核":"儲存修改內容"}]})};});},
    ...
    setOutboundStatus(id:string,status:"已取消"|"已結案"|"已刪除",user:User){setStore(s=>{const outbound=s.outbounds.find(x=>x.id===id);if(!outbound)return s;const shouldRestore=outbound.status==="已生效"&&status!=="已結案",allocations=outbound.lines.flatMap(l=>l.allocations);const consignments=shouldRestore?s.consignments.map(c=>{const lines=c.lines.map(l=>{const restore=allocations.filter(a=>a.consignmentLineId===l.id).reduce((n,a)=>n+a.qty,0);return restore?{...l,remainingQty:l.remainingQty+restore}:l;});return allocations.some(a=>a.consignmentId===c.id)?{...c,lines,status:"生效" as Status,audits:[...c.audits,{id:makeId("a"),action:"出庫沖銷",actorId:user.id,actorName:user.name,at:new Date().toISOString(),note:`${outbound.no} 狀態改為 ${status}，恢復寄庫量`}]}:c;}):s.consignments;return{...s,consignments,outbounds:s.outbounds.map(x=>x.id!==id?x:{...x,status,audits:[...x.audits,{id:makeId("a"),action:status,actorId:user.id,actorName:user.name,at:new Date().toISOString(),note:shouldRestore?"已沖銷並恢復寄庫餘量":"單據保留，僅變更狀態"}]})};});},
